/**
 * FinSphere Financial Management Platform — Backend (Google Apps Script)
 * ------------------------------------------------------------------
 ...
 * the HTML app's built-in connection.
 *
...
const HEADERS = {
  ...
  // references: JSON string, array of guarantor profiles. Photos are compact
  // data URLs produced by the frontend (small enough for a Sheet cell).
  // — kept as one JSON field rather than a separate sheet since it's
  // small, bounded (2-3 entries), and always read/written as a whole
  // alongside the borrower record, never queried independently.
  Borrowers: ['id', 'name', 'fatherName', 'address', 'mobile', 'photo', 'remarks', 'references', 'status', 'createdAt'],
  AuditLog: ['id', 'action', 'entity', 'entityId', 'details', 'createdAt'],
  Counters: ['name', 'value']
};
  ...
  try {
    ...
    switch (action) {
      ...
      case 'deleteInvestor':
        ...
        break;
      case 'addBorrower':
        result = addBorrower(body.payload);
        break;
      case 'editBorrower':
        result = editBorrower(body.payload);
        break;
      case 'setBorrowerStatus':
        result = setBorrowerStatus(body.payload);
        break;
      case 'deleteBorrower':
        result = deleteBorrower(body.payload);
        break;
      case 'addInvestment':
...
function editInvestor(p) {
  const updates = {};
  ['name', 'fatherName', 'address', 'mobile', 'photo', 'remarks'].forEach(k => {
...
// warning the admin first when history exists — this function itself
...
// ---------- Borrower operations ----------
// Mirrors the Investor pattern above exactly, on the Borrowers sheet.
// The Loans/EMI/Bullet repayment engine is a separate, larger piece
// not yet built — this is the borrower master-record CRUD only
// (add/edit/status/delete), same as Investors gives you the investor
// master record before any accounts exist under them.

function addBorrower(p) {
  const seq = nextCounter('borrower');
  const id = 'BRW' + String(seq).padStart(2, '0');
  const record = {
    id: id,
    name: p.name,
    fatherName: p.fatherName || '',
    ...
    mobile: p.mobile,
    remarks: p.remarks || '',
    references: JSON.stringify(Array.isArray(p.references) ? p.references : []),
    status: 'ACTIVE',
    createdAt: new Date().toISOString()
  };
  appendRow(SHEET_NAMES.BORROWERS, record);
  logAudit('CREATE', 'Borrower', id, record);
  return { ok: true, borrower: record };
}

function editBorrower(p) {
  const updates = {};
  ['name', 'fatherName', 'address', 'mobile', 'remarks'].forEach(k => {
    if (p[k] !== undefined) updates[k] = p[k];
  });
  if (p.references !== undefined) {
    updates.references = JSON.stringify(Array.isArray(p.references) ? p.references : []);
  }
  const success = updateRowById(SHEET_NAMES.BORROWERS, p.id, updates);
  if (!success) return { error: 'Borrower not found' };
  logAudit('EDIT', 'Borrower', p.id, updates);
  return { ok: true };
}

function setBorrowerStatus(p) {
  const success = updateRowById(SHEET_NAMES.BORROWERS, p.id, { status: p.status });
  if (!success) return { error: 'Borrower not found' };
  logAudit('STATUS_CHANGE', 'Borrower', p.id, { status: p.status });
  return { ok: true };
}

// Same deletion policy as deleteInvestor — always allowed, frontend
// is responsible for confirming with the admin first when the
// borrower has loan history (once the Loans module exists).
function deleteBorrower(p) {
  const deleted = deleteRowById(SHEET_NAMES.BORROWERS, p.id);
  if (!deleted) return { error: 'Borrower not found' };
  logAudit('DELETE', 'Borrower', p.id, deleted);
  return { ok: true };
}
...
//    topUpAccountId (optional — if set, adds to this existing account
...
// in place but with no transactions — recalculation will simply show
...
function getAllData() {
  ...
  const accounts = readRows(SHEET_NAMES.ACCOUNTS);
  const transactions = readRows(SHEET_NAMES.TRANSACTIONS);
  const borrowers = readRows(SHEET_NAMES.BORROWERS).map(b => {
    let refs = [];
    try { refs = b.references ? JSON.parse(b.references) : []; } catch (e) { refs = []; }
    return Object.assign({}, b, { references: refs });
  });
...
// transactions that occurred on or before the cutoff date — giving a
