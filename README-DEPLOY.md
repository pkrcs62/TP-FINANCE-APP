# TP Finance v4.00 — Supabase Deployment Guide
...
## ⚠️ Upgrading an existing V3.8x (or earlier) database to V4.00

V4.00 replaces the separate Admin-password and Field-Agent-PIN login
screens with one email + OTP login for everyone, adds a User Management
mode for approving new signups and linking Investor/Borrower accounts to
their real records, and retones Investor Mode's colors. This needs new
tables and functions in Supabase before the new `index.html` will work —
if you only swap the app file without running the SQL below, the login
screen will load but sending/verifying a code will fail.

Open **SQL Editor → New query** and run this once (safe to re-run):

```sql
-- User profiles: staff + investor/borrower logins
create table if not exists user_profiles (
  id uuid primary key default gen_random_uuid(),
  email text unique not null,
  display_name text not null,
  role text not null check (role in ('DEVELOPER','ADMIN','AGENT','INVESTOR','BORROWER')),
  linked_agent_id text references agents(id),
  linked_investor_id text,
...
alter table user_profiles add constraint user_profiles_linked_investor_id_key unique (linked_investor_id);
alter table user_profiles add constraint user_profiles_linked_borrower_id_key unique (linked_borrower_id);

-- OTP codes for login/signup verification
create table if not exists otp_codes (
  id uuid primary key default gen_random_uuid(),
  email text not null,
  code text not null,
  purpose text not null default 'LOGIN' check (purpose in ('LOGIN','SIGNUP','EMERGENCY')),
...
Then paste in the `request_login_otp`, `verify_login_otp`,
`request_login_otp_p`, `verify_login_otp_p`, `list_pending_users_p`,
`approve_user_p`, `reject_user_p`, `search_investors_p`,
`search_borrowers_p`, and `generate_emergency_otp` function blocks and
run that too.

**Seed your first admin accounts** so someone can actually log in and
approve everyone else — replace with real emails and run once:

```sql
insert into user_profiles (email, display_name, role, status, approved_by, approved_at)
values
  ('you@example.com', 'Your Name', 'DEVELOPER', 'ACTIVE', 'SYSTEM', now()),
  ('admin2@example.com', 'Second Admin', 'ADMIN', 'ACTIVE', 'SYSTEM', now())
on conflict (email) do nothing;
```

**Locked out with no way to log in?** Run this directly in SQL Editor
(never from inside the app) to get a real, single-use, 30-minute code
for a specific existing account:

```sql
select generate_emergency_otp('you@example.com');
```
...
## v1.59 Bug Fixes & Refinements (deploy notes)

This release is on top of v1.58 and adds **`04_amendments_v3.sql`** —
a small additional migration containing two changes: `approve_agent_repayment()`
now accepts an admin-chosen pool allocation (forwarded to the same repayment
logic every other repayment uses) instead of posting with no pool choice at
all, and a new `get_borrower_credit_rating_detail()` function powers the
"why this score" popup on the Borrower Profile gauge. Run it after
`03_amendments_v2.sql`.

**What changed, in plain terms:**

1. **Borrowers screen Submit was broken** — pressing Submit silently did
   nothing, in every case. Fixed.

2. **Duplicate village filter removed** — the Borrowers results list had
   two independent village filters (the new dropdown gate, and an old
   leftover pill row) that could disagree with each other. Only the gate
   remains.

3. **"With Loan" / "Without Loan" filter** — a new chip filter on the
   Borrowers results list, to see only borrowers who currently have a
   loan account, or only those who don't.

4. **"By Bahi No" lookup on Loans** — a third option alongside "By Loan
   Type" and "By Village". Pick EMI, Bullet, or Overdraft, type in just
   the plain number from the physical register, and it finds that exact
   loan (e.g. typing 12 under EMI finds loan E-12).

5. **Date Filter on Loans** — a small, collapsed-by-default "📅 Date
   Filter" control. Lets you search by a date range on either
   Disbursement Date or Sanction Date (pick one via a toggle), and works
   whether or not any other filter is also selected.

6. **Agent-Collected Repayments now show full context before you decide**:
   - **Approve** used to post the instant you tapped it, with no chance to
     review anything. It now opens a proper screen showing the loan's
     current position, the agent's reported amount/date/mode/remarks, and
     requires you to choose which pool to credit — exactly like recording
     a normal repayment. After confirming, you get the same "credited to"
     popup shown everywhere else in the app.
   - **Reject** now shows the loan, borrower, amount, and agent name at
     the top of the reject screen, so you don't have to remember the
     details from the list before typing a reason.

7. **Borrower Credit Rating is now genuinely visible** — a colourful,
   animated semicircular gauge appears on every Borrower Profile screen
   (it was missing there entirely in v1.58, only shown as a small text
   line on Loan Details). The gauge's colour, icon, and glow change with
   the score band (Poor/Fair/Good/Excellent). Tapping the gauge opens a
   plain-English breakdown: which specific months were paid late, by how
   many days, across every loan that borrower has ever had.

8. **"Download PDF" renamed to "Download Profile"** on the Borrower
   Profile screen, since that's what it actually downloads.

As with v1.58, the service worker's cache version has been bumped
(`finsphere-v1-59`) so existing installs update automatically the next
time they're opened.
