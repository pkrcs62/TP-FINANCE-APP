/**
 * T.P. Finance PWA — Service Worker
 * ---------------------------------
 * Strategy:
...
const CACHE_VERSION = 'tpfinance-v3';
const APP_SHELL = [
  './',
  './index.html',
  './config.js',
...
self.addEventListener('fetch', (event) => {
  ...
  if (isLiveDataRequest(url)) {
    event.respondWith(fetch(req).catch(() => new Response(
      JSON.stringify({ error: 'Database not connected. Please check your Settings and the Apps Script Web App URL, then try again.' }),
