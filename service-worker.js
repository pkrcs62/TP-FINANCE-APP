...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => (k.startsWith('finsphere-') || k.startsWith('tpfinance-')) && k !== CACHE_VERSION).map((k) => caches.delete(k))
      )
